...
const FLAGS_V2: RoadmapStepV2[] = [
  ...
  },
  {
    ...
    docs: [{ label: 'Targeting and segments', href: 'https://launchdarkly.com/docs/home/flags/target' }],
    learn: {
      what: 'Target individual contexts, attribute rules, or a percentage rollout. The dimensions you target on (user, account, device) are context kinds; they come from the contexts your app sends, defined in the SDK snippet rather than here, and your setup seeds a couple of examples (account, device) to start. Build a reusable segment when you want the same audience across more than one flag. No segment is required to start.',
...
const EXPERIMENTS_V2: RoadmapStepV2[] = [
  {
    key: 'scaffold',
    title: 'Run the sample experiment first',
    ...
    est: '~2 min',
    learn: {
      what: 'See one work end to end before you build your own. LaunchDarkly scaffolds a complete experiment (a flag with two variations, a conversion metric, and a 50/50 test) and walks you through running it. The classic sample tests which button copy converts better. Once you’ve done it, the steps below are how you set up your own.',
      ideas: ['Button copy', 'Checkout flow', 'Onboarding steps', 'Pricing page'],
    },
  },
  {
    key: 'flag',
    title: 'Create the flag you’ll test',
    blurb: 'Its variations become the treatments.',
    cta: 'Create flag',
    icon: 'flag',
    est: '~2 min',
    learn: {
      what: 'Now set up your own. Any flag can power an experiment, and its variations become the things you compare. Use a boolean for an on/off test or a multivariate flag for an A/B/n, on the change you want to measure.',
      ideas: ['Boolean A/B', 'Multivariate A/B/n', 'String variations'],
    },
  },
  {
    key: 'metric',
    ...
    docs: [{ label: 'Creating metrics', href: 'https://launchdarkly.com/docs/home/metrics/create-metrics' }],
    learn: {
      what: 'Pick the outcome you’re optimizing. Click and page-view metrics need no code (a browser SDK sends them from a CSS selector or URL); conversion and numeric metrics are one track() call when the outcome happens.',
      ideas: ['Click / page view', 'Conversion', 'Revenue per visitor'],
    },
  },
  {
    key: 'build',
    title: 'Build the experiment',
    blurb: 'Hypothesis, randomization unit, control. Stats preset.',
    cta: 'Create experiment',
    icon: 'fingerprint',
    est: '~5 min',
    docs: [{ label: 'Creating experiments', href: 'https://launchdarkly.com/docs/home/experimentation/create' }],
    learn: {
      what: 'Name it, write the hypothesis, choose what you randomize by (the context kind: user works by default; a custom unit like account has to be turned on for experiments first), attach your metric, pick the flag and the targeting rule to run on, set the split, and designate the control. Statistics default to Bayesian. You can design the whole thing in the UI and save it before writing any code.',
      ideas: ['Randomize by user', '50/50 split', 'Bayesian default'],
    },
  },
  {
    key: 'instrument',
    title: 'Instrument your app to serve and measure it',
    blurb: 'Evaluate, track, and flush, with one context kind.',
    cta: 'See the code',
    icon: 'plug',
    est: '~10 min',
    docs: [{ label: 'Experiment events', href: 'https://launchdarkly.com/docs/home/experimentation/events' }],
    learn: {
      what: 'Calling variation() for the flag enrolls each context automatically, so evaluate it for everyone the rule targets and do not filter the audience in code, or you will skew the results. Send the metric event with track() (click and page-view need no call), keep the same context kind across the evaluation and the event or it will not attribute, identify() the context on login, and flush() before any redirect so click events are not dropped.',
      ideas: ['variation() = exposure', "track('purchase')", 'Match the context kind', 'flush() before redirect'],
    },
  },
  {
    key: 'start',
    title: 'Toggle on and start the iteration',
    blurb: 'Stats are preset. Results need volume.',
    cta: 'Start iteration',
    icon: 'play',
    est: '~1 min',
    learn: {
      what: 'Toggle the flag on (an experiment only records data while the flag is on), then start the iteration and watch results roll in. Give it real event volume before reading too much into early numbers; a fixed-horizon frequentist test wants a sample size up front.',
      ideas: ['Flag on', 'Live results', 'Ship the winner'],
    },
  },
  {
    key: 'segment',
    title: 'Scope the audience to a segment',
    blurb: 'Run on a targeting rule, not all traffic.',
    cta: 'Open targeting',
    icon: 'venn',
    optional: true,
    est: '~3 min',
    learn: {
      what: 'Instead of the default rule, run the experiment on a targeting rule, including one that references a reusable segment like a beta cohort, plan tier, or region. Always scope the audience with the rule, never in app code.',
      ideas: ['Beta cohort', 'Plan tier', 'Region'],
    },
  },
  {
    key: 'holdout',
